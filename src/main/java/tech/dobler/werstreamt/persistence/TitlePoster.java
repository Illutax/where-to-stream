 ...
 * werstreamt.es availability cache). Holds the TMDB {@code poster_path} plus the small thumbnail
 * and the larger hover image as BLOBs — both fetched lazily (thumbnail on first view, full on first
 * hover) and stored here so TMDB is hit at most once per title. A {@code null posterPath} is a
 * negative cache entry (TMDB had no poster), re-checked only after a TTL.
 */
@Entity
