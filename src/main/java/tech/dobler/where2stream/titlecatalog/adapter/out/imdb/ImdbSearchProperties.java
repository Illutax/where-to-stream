...
/**
 * Binding for the {@code imdb-search.*} configuration —
 * the navbar title search, backed by IMDb's public (no-auth) suggestion/typeahead endpoint
 * (the same JSON API IMDb's own site search box uses).
 * Outbound requests are throttled (default 5 req/s) —
 * a single global limit shared across every user's search
 * (the client-side debounce keeps a single well-behaved browser tab well under this;
 * the limit's job is bounding the *aggregate* rate across all concurrent users,
 * not re-implementing the debounce server-side).
 *
...
public record ImdbSearchProperties(
     ...
     *                          in aggregate across every user (≤ 0 disables throttling)
     */
