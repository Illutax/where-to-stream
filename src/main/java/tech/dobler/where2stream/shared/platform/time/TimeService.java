package tech.dobler.where2stream.shared.platform.time;
 ...
 * (never {@link Instant#now()} / {@link LocalDate#now()} directly)
 * so that tests can substitute a fixed clock and assert against exact, repeatable timestamps.
 */
