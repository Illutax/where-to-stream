...
/**
 * A title's release year. A value object over the former bare {@code int}: it owns the domain rule
 * that year {@code 0} marks a title with no release year yet ("Not yet released"). Serialises to/from
 * a plain JSON number ({@link JsonValue} / {@link JsonCreator}) so the {@code /api} contract for the
 * numeric year is unchanged.
 */
